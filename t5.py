    ...
    try:
        ...
        if df.empty:
            ...
            if not filtered_df.empty:
                ...
                # Részletes eredmények minden diákra expanderekben
                st.subheader("📋 Részletes eredmények")
                
                for index, row in filtered_df.iterrows():
                    student_display_name = f"{row['student_name']} ({row['class']}) - {row['timestamp']}"
                    with st.expander(student_display_name, expanded=False):
                        display_student_result(row.to_dict(), quiz_data)
